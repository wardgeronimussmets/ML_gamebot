...
def show_multiple_images(images,image_types):
    # Create a figure with two subplots arranged side by side
    fig, axs = plt.subplots(1, len(images))
    
    for x in range(len(images)):
        if image_types == GRAY_IMAGE:
            axs[x].imshow(images[x], cmap='gray')

        elif image_types == RGB_IMAGE:
            axs[x].imshow(images[x])

        elif image_types == BGR_IMAGE:
            im = cv2.cvtColor(images[x],cv2.COLOR_BGR2RGB)
            axs[x].imshow(im)
            plt.title("carefull there are the RGB values not the BGR values")

        axs[x].set_title('Image '+str(x))
